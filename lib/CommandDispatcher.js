...
class CommandDispatcher {
  ...
  async getCompletionSuggestions (parse, cursor = parse.reader.getTotalLength()) {
    ...
    }).catch(() => {}) // Errors are currently ignored to have the same behavior as brigadier (maybe this should be changed?)
  }
  ...
  #addPaths (node, result, parents) {
    const current = Array.from(parents)
    current.push(node)
    result.push(current)

    for (const child of node.getChildren()) this.#addPaths(child, result, current)
  }
}
